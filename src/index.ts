...
export function createAiChatWidget(
  ...
  //
  // Lifted whole to `./job-cards` (#320) along this banner. 559 lines that own
  // their own state — what is being watched, which polls are running, what the
  // user confirmed this session — and expose four things.
  //
  // The forwarding arrows below are not ceremony. `frameShowing`,
  // `autoNavigateFrame` and `dispatchAction` are declared BELOW this point;
  // passing them by value here would read them in their temporal dead zone.
  // They are only ever called after mount, so deferring the reference is
  // enough. `getThreadId` is a getter for a different reason: a job is polled
  // for minutes and outlives a thread switch, so a captured id would match a
  // finished job against the thread the user was in when it started.
  const { reattachTrackedJobs, subscribeJob, trackConfirmedJob, hasUserConfirmed } =
    createJobCards({
      L,
      log,
      bubble,
      ...
      toggle: () => toggle(),
      waitAlive,
      frameShowing: (path) => frameShowing(path),
      autoNavigateFrame: (key, path) => autoNavigateFrame(key, path),
      dispatchAction: (action, data) => dispatchAction(action, data),
  ...
  const autoNavigateFrame = (key: string, path: string): void => {
    ...
    if (
      !shouldAutoNavigate({
        advanced,
        canNavigate: true,
        userConfirmedThisSession: hasUserConfirmed(key),
